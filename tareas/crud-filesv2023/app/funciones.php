...
{
    $fich = @fopen(FILEUSER, 'w') or die("ERROR al grabar el fichero de usuarios"); // abrimos el fichero para escritura
    
    foreach ($tvalores as $login => $datosusuario){
        $linea = $login."|";
        $linea .= implode('|',$datosusuario);
        $linea .="\n";
        fputs($fich,$linea);
    }
...
function volcarDatoscsv(array $tvalores)
{
    ...
    foreach ($tvalores as $login => $datosusuario){
        // Tenemos que añadir el lógin en la tabla
        array_unshift($datosusuario,$login);
        fputcsv($fich,$datosusuario);
    }
