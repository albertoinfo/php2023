...
class AccesoDatos {
    ...
    // Obtengo el máximo de puntos
    public function obtenerMaxPuntos (){
        
        $stmt = $this->dbh->query("select max(puntos) as maxpuntos from clientes");
        // Por defecto devuelve una tabla asociativa
        $result = $stmt->fetch();
        $maxpuntos = $result['maxpuntos'];
        return $maxpuntos;
